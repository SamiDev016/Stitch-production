    ...
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frm.doc.finish_goods.forEach((fg, idx) => {
                ...
                if (main_batch) {
                    // ✅ Store original qty in temp field
                    if (!main_batch.original_parts_qty) {
                        // main_batch.original_parts_qty = main_batch.parts_qty || 0;
                        // frappe.model.set_value(main_batch.doctype, main_batch.name, "original_parts_qty", main_batch.parts_qty || 0);
                        main_batch.original_parts_qty = main_batch.parts_qty || 0;
                    }
                ...
                for (const key of Object.keys(group_map)) {
                    ...
                    group.sort((a, b) => a.batch.localeCompare(b.batch));

                    const total_in_group = group.reduce((sum, ob) => sum + (ob.qty || 0), 0);
                    if (total_in_group <= new_qty) continue;
            ...
            frm.doc.other_batches = frm.doc.other_batches.filter(ob => ob.qty > 0);
